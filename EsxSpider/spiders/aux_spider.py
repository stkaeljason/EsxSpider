...
class Aux_spider(scrapy.Spider):
    ...
    def start_requests(self):
        # 从数据库获取商家账号信息
        merchant_info = query_aux_merchant()
        ...
        for account in account_list:
            print('merchant_start:%s'%account['username'])
    ...
    def after_login(self, response):
        ...
        # 测试用详情提取，之后注释或者删除
        # test_url = 'http://aux.bangjia.me/index.php/Auxtest/detail/orderno/2017121329828648133601'
        # return Request(test_url, meta={'cookiejar':response.meta['cookiejar'],'account_info': response.meta['account_info']},
        ...
        yield FormRequest(url=self.website_jiedai,
                      ...
                      dont_filter=True)
        # # 构造师傅数据请求
        # yield Request(self.jjshifu_url,
        #               meta={'cookiejar': response.meta['cookiejar'], 'account_info': response.meta['account_info']},
        #               callback=self.master_parse, dont_filter=True, errback=self.report_errror)
    ...
    def work_order_parse(self, response):
        ...
        # print response.meta['account_info']
    ...
    def order_detail_parse(self, response):
        ...
        try:
            ...
            order_detail['ord_cust_lng'], order_detail['ord_cust_lat'] = get_latlng(order_detail['ord_city'], order_detail['ord_district']+order_detail['ord_street']+order_detail['ord_cust_addr'])
